...
const { UnauthorizedError, ValidationError, NotFoundError } = require("../exceptions/errors");
...
exports.acceptFriend = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      successResponse(res, "Friend request accepted but conversation creation failed", {
        friendship, 
...
exports.cancelFriendRequest = async (req, res, next) => {
  ...
  try {
    ...
    if (io) {
      io.to(`user_${friendId}`).emit("friend_request_canceled", { from: userId });
    }
    
    successResponse(res, "Friend request canceled successfully", { success: true });
  } catch (error) {
    next(error);
  }
};
...
exports.getSentFriendRequests = async (req, res, next) => {
  ...
  try {
    ...
    successResponse(res, "Sent friend requests fetched successfully", sentRequests);
  } catch (error) {
    next(error);
  }
};
...
exports.getFriendSuggestions = async (req, res, next) => {
  ...
  try {
    ...
    successResponse(res, "Friend suggestions fetched successfully", suggestions);
  } catch (error) {
    next(error);
  }
};
...
exports.checkConversationMembers = async (req, res, next) => {
  ...
  try {
    console.log(`Checking conversation_members for conversation: ${conversationId}`);
    ...
    console.log(`Found ${members.length} members in conversation_members table`);
