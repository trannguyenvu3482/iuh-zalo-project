...
const setupCallHandlers = (io, socket, user) => {
  console.log(`Setting up call handlers for user ${user.id}, socket ${socket.id}`);

  // Handle call initiation
  socket.on('call:initiate', async (data, callback) => {
    const { calleeId, callerId, callerName, type, channelName: clientChannelName, token } = data;
    ...
    if (callerId !== user.id) {
      console.error(`User ID mismatch: socket user ${user.id} tried to initiate call as ${callerId}`);
      if (typeof callback === 'function') {
        callback({ 
          success: false, 
    ...
    console.log(`Rooms for callee ${calleeId}:`, 
      calleeRooms ? { size: calleeRooms.size, exists: true } : { exists: false });
    
    // Emit to the callee
    socket.to(`user_${calleeId}`).emit('call:incoming', callData);
    
    console.log(`Call initiated from ${callerId} to ${calleeId} on channel ${channelName}`);
  ...
  socket.on('call:accept', async (data) => {
    ...
    const calleeName = user?.fullname || user?.name || user?.username || 'User';
    ...
    console.log(`Call accepted: ${callerId} and ${calleeId} in channel ${finalChannelName}`);
  });
